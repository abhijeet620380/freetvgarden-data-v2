...
//      have. A Famelack channel is NEVER merged into or replaced by an
//      iptv-org channel just for sharing a name+country — channel names
//      repeat across genuinely different regional feeds (e.g. "Zee
//      Cinema" India vs "Zee Cinema HD" Singapore/APAC), so collapsing
//      by name risked silently losing a real, distinct broadcast. Only
//      an EXACT triple match (same name + country + url) is skipped, as
//      that really is the same entry, not a different channel.
...
// scraped upstream source. A name containing one of these telltale
// substrings isn't a real channel name — but a bad name says nothing
// about whether the STREAM works, so this only replaces the name (see
// where it's used below); it never removes the channel. A long name on
// its own is NOT treated as garbage — plenty of real channel names are
// long, and that alone doesn't mean anything is wrong with them.
...
async function main() {
  ...
  // Famelack channels are no longer health-checked at all — trusted as
  // always "live", the same treatment the curated YOUTUBE_LIVE list and
  // isYouTube channels already got. This was previously only true for
  // isYouTube entries; now it applies to every Famelack channel, so only
  // iptv-org's channels actually get network-checked each run.
  console.log(`Skipping health checks for ${famelackChannels.length} Famelack channels (trusted as always live) — only iptv-org channels get checked.`);
  ...
  // Every iptv-org and Famelack channel is kept as its own separate entry
  // — NEVER collapsed or overwritten just because another channel shares
  // its name+country. Channel names repeat constantly across genuinely
  // different feeds (a regional "Zee Cinema" vs a Singapore/APAC "Zee
  // Cinema HD" that happen to normalize to a similar key, a channel with
  // multiple independent mirrors, etc.) — treating a shared name as "the
  // same channel" and replacing one with the other risks silently losing
  // a real, distinct broadcast. The only thing actually skipped below is
  // an EXACT triple match (same name + country + url) — that really is
  // the same entry, not a different channel.
  const seenExact = new Set();
  const allCheckedRaw = [];
  let famelackAddedNew = 0, exactDuplicatesSkipped = 0;
  for (const ch of iptvOrgResults) {
    const exactKey = `${ch.name}|${ch.country}|${ch.url}`.toLowerCase();
    if (seenExact.has(exactKey)) { exactDuplicatesSkipped++; continue; }
    seenExact.add(exactKey);
    allCheckedRaw.push(ch);
  }
  ...
  console.log(`Famelack: ${famelackAddedNew} channels added (kept as separate entries, never merged into a same-named iptv-org channel). ${exactDuplicatesSkipped} exact duplicates (identical name+country+url) skipped.`);
  ...
  // Every output list gets sorted alphabetically by name — otherwise
  // channels just publish in fetch order (iptv-org's channels first, then
  // every newly-added Famelack channel appended after, unsorted relative
  // to the rest), which is why YouTube-badge channels were clustering at
  // the end of lists instead of interleaving A-Z with everything else.
