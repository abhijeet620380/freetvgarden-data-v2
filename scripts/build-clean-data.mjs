...
async function fetchFamelackSupplementary(categoriesData) {
  ...
  try {
    const [res, nanoidToCategoryId] = await Promise.all([
      fetch(url),
    ...
    console.log(`Famelack: parsed ${collected.length} channels (from ${data.length} raw entries), ${categorized} matched to a real category (rest default to General).`);
    return collected;
  } catch (e) {
