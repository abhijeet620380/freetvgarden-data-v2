...
async function main() {
  ...
  for (const ch of iptvOrgResults) {
    if (ch.id && ch.id.includes('@')) {
      const [baseId, tag] = ch.id.split('@');
      const upperTag = tag.toUpperCase();

      if (['SD', 'HD', 'FHD', 'UHD', '4K', '720P', '1080P'].includes(upperTag)) {
        // 1. It is a quality tag. Strip it so duplicate SD/HD feeds merge into one clean channel.
        ch.id = baseId;
      } else {
        // 2. It is a regional tag (like APAC). Keep the ID unique and rename the channel.
        if (!String(ch.name).toUpperCase().includes(upperTag)) {
          ch.name = `${ch.name} ${upperTag}`;
        }
      }
    }

    // Create the deduplication key AFTER modifying the name.
    // Now, "Zee Cinema" and "Zee Cinema APAC" create two completely different keys.
