...
async function fetchLanguageNameMap() {
  // Radio's languages field uses the same ISO 639-2/3 codes as iptv-org
  // (confirmed: "cat", "eng", "hin" etc. match exactly) — reusing their
  // public language list avoids hand-maintaining a duplicate mapping.
  try {
    const res = await fetch("https://iptv-org.github.io/api/languages.json", { signal: AbortSignal.timeout(15000) });
    if (!res.ok) return new Map();
    const list = await res.json();
    return new Map(list.map(l => [l.code, l.name]));
  } catch (e) {
    console.log(`  Couldn't fetch language names (${e.message}) — language field will show raw codes instead of full names.`);
    return new Map();
  }
}

function normalizeEntry(raw, categoryId, categoryName, sourceField, langNameByCode) {
  ...
  return {
    ...
    language: (raw.languages && raw.languages[0]) ? (langNameByCode.get(raw.languages[0].toLowerCase()) || raw.languages[0].toUpperCase()) : "",
...
async function buildOne(type, categoryIds, sourceField, langNameByCode) {
  console.log(`\n=== Building ${type} ===`);
  ...
  for (const raw of data) {
    ...
    const entry = normalizeEntry(raw, categoryId, categoryId, sourceField, langNameByCode);
    if (entry) {
  ...
  for (const ch of channels) {
    ...
    if (ch.language) {
      const langKey = String(ch.language).toLowerCase().replace(/\s+/g, "-");
      (byLanguage[langKey] = byLanguage[langKey] || []).push(ch);
    }
  }
  ...
  // Language grouping — new. Note: only channels with a real language
  // value (fixed this run) get grouped; webcams frequently have no
  // language at all in Famelack's own data, so this folder may end up
  // small or empty for webcams — that's accurate, not a bug.
  await fs.mkdir(path.join(outDir, "languages"), { recursive: true });
  for (const [lang, list] of Object.entries(byLanguage)) {
    await fs.writeFile(path.join(outDir, "languages", `${lang}.m3u`), toM3U(sortByName(list)));
  }
  await fs.writeFile(
    path.join(outDir, "languages_metadata.json"),
    JSON.stringify(
      Object.entries(byLanguage)
        .map(([lang, list]) => ({ language: lang, count: list.length }))
...
async function main() {
  const langNameByCode = await fetchLanguageNameMap();
  await buildOne("radio", RADIO_CATEGORY_IDS, "streams", langNameByCode);
  await buildOne("webcams", WEBCAM_CATEGORY_IDS, "youtube", langNameByCode);
  console.log("\nDone.");
}
